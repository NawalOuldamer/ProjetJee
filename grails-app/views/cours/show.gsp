	...
	<body>
		...
		<div id="show-cours" class="content scaffold-show" role="main">
			...
			<ol class="property-list cours">
				...
				<g:if test="${coursInstance?.utilisateur}">
				<li class="fieldcontain">
					<span id="utilisateur-label" class="property-label"><g:message code="cours.utilisateur.label" default="Utilisateur" /></span>
					
						<g:each in="${coursInstance.utilisateur}" var="u">
						<span class="property-value" aria-labelledby="utilisateur-label"><g:link controller="user" action="show" id="${u.id}">${u?.encodeAsHTML()}</g:link></span>
						</g:each>
