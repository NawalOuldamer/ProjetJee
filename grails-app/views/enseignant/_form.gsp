<%@ page import="projetjee.profile.Enseignant" %>
...
<div class="fieldcontain ${hasErrors(bean: enseignantInstance, field: 'password', 'error')} ">
	...
	<g:textField name="password" maxlength="10" value="${enseignantInstance?.password}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: enseignantInstance, field: 'question', 'error')} ">
	...
	<g:select name="question" from="${projetjee.question.Question.list()}" multiple="multiple" optionKey="id" size="5" value="${enseignantInstance?.question*.id}" class="many-to-many"/>
</div>
